...
{
	ValueTreeIterator::forEach(v, ValueTreeIterator::OnlyChildren, [&](ValueTree& v)
	{
...
ParameterEncoder::ParameterEncoder(jit::ComplexType::Ptr jitCompiledMetadata)
{
	if (auto st = dynamic_cast<StructType*>(jitCompiledMetadata.get()))
	{
		auto list = st->makeDefaultInitialiserList();
		...
		list->forEach([&bytes](InitialiserList::ChildBase* p)
		{
			...
			if (p->getValue(v))
			{
				...
				bytes.add(iData);
//				s << String::toHexString(iData).toUpperCase();
			}
...
void ParameterEncoder::writeIntBlock(Base& b)
{
	...
	Macro(b, "SNEX_METADATA_ENCODED_PARAMETERS", { String(numIntsWritten) }, false);
...
{
	index = ValueTreeIterator::getIndexInParent(v);
