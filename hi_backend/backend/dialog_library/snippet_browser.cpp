...
author: "")" }, 
	...
	auto& List_19 = List_0.addChild<List>({
	  { mpid::Text, "Tags" }, 
	  { mpid::Foldable, 1 }, 
	  { mpid::Folded, 1 }
	});

	List_19.addChild<TagList>({
	  { mpid::ID, "tagList" }, 
	  { mpid::Items, R"(API
Best Practice
Broadcaster
...
rebuildTable();
	...
	auto& List_25 = mp.addPage<List>({
	  { mpid::Style, "gap: 10px; " }, 
	  { mpid::Class, ".setup-page" }
	});

	auto& Column_26 = List_25.addChild<Column>({
	  { mpid::Class, ".header-label" }
	});

	Column_26.addChild<SimpleText>({
	  { mpid::Text, "Snippet Browser" }, 
	  { mpid::Class, "#headerlabel" }, 
	  { mpid::Style, "flex-grow: 1;" }
	});

	Column_26.addChild<Button>({
	  { mpid::ID, "applyButton" }, 
	...
	List_25.addChild<PersistentSettings>({
	  { mpid::ID, "snippetBrowser" }, 
	  { mpid::Filename, "snippetBrowser" }, 
	  { mpid::UseChildState, 1 }, 
	  { mpid::Items, R"(snippetDirectory:""
author: "")" }, 
	...
	List_31.addChild<SimpleText>({
	  { mpid::Text, "Username" }, 
	  { mpid::Style, "text-align: left; width: 100%;margin-top: 10px;" }
	});

	List_31.addChild<TextInput>({
	  { mpid::ID, "author" }, 
	...
	auto& List_38 = List_25.addChild<List>({
	...
	// Custom callback for page List_25
	List_25.setCustomCheckFunction([](Dialog::PageBase* b, const var& obj){

		return Result::ok();

	});
	
	auto& List_47 = mp.addPage<List>({
	  { mpid::Style, "height: 100%;" }
	});

	auto& Column_48 = List_47.addChild<Column>({
	  { mpid::Class, ".header-label" }
	});

	Column_48.addChild<SimpleText>({
	  { mpid::Text, "Add Snippet" }, 
	  { mpid::ID, "editTitle" }, 
	  { mpid::Style, "flex-grow: 1;" }
	});

	Column_48.addChild<Button>({
	  { mpid::ID, "closeButton" }, 
	...
	auto& Column_53 = List_51.addChild<Column>({
	  { mpid::Style, "gap:1px;height: auto;" }
	});

	Column_53.addChild<Button>({
	  { mpid::Text, "All" }, 
	  { mpid::ID, "addCategory" }, 
	  { mpid::InitValue, "0" }, 
	  { mpid::UseInitValue, 1 }, 
	...
	auto& List_60 = List_51.addChild<List>({
	  { mpid::Text, "Tags" }, 
	  { mpid::Foldable, 1 }, 
	  { mpid::Folded, 1 }
	});

	List_60.addChild<TagList>({
	  { mpid::ID, "addTagList" }, 
	  { mpid::Items, R"(API
Best Practice
Broadcaster
...
5 - great stuff)" }
	...
	List_51.addChild<MarkdownText>({
	  { mpid::ID, "descriptionPreview" }, 
	  { mpid::Class, ".description" }, 
	  { mpid::Style, "font-size: 18px; height: 180px;" }
	});

	List_51.addChild<Button>({
	  { mpid::Text, "Save file" }, 
	...
	List_51.addChild<JavascriptFunction>({
	  { mpid::CallOnNext, 0 }, 
	  { mpid::Code, R"(
...
document.getElementById("description").addEventListener("change", function()
{
	...
	List_47.addChild<JavascriptFunction>({
	  { mpid::CallOnNext, 1 }, 
...
if(state.newName.length > 0)
{
	...
	// Custom callback for page List_47
	List_47.setCustomCheckFunction([](Dialog::PageBase* b, const var& obj){
