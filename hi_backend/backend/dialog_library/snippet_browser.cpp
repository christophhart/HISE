...
author: "")" }, 
	...
	auto& List_39 = List_26.addChild<List>({
	...
	auto& List_48 = mp.addPage<List>({
	  { mpid::Style, "height: 100%;" }
	});

	auto& Column_49 = List_48.addChild<Column>({
	  { mpid::Class, ".header-label" }
	});

	Column_49.addChild<SimpleText>({
	  { mpid::Text, "Add Snippet" }, 
	  { mpid::ID, "editTitle" }, 
	  { mpid::Style, "flex-grow: 1;" }
	});

	Column_49.addChild<Button>({
	  { mpid::ID, "closeButton" }, 
	...
	List_52.addChild<MarkdownText>({
	  { mpid::Text, "Please select a category and tags for the snippet" }
	});

	auto& Column_55 = List_52.addChild<Column>({
	  { mpid::Style, "gap:1px;height: auto;" }
	});

	Column_55.addChild<Button>({
	  { mpid::Text, "All" }, 
	  { mpid::ID, "addCategory" }, 
	  { mpid::InitValue, "0" }, 
	  { mpid::UseInitValue, 1 }, 
	...
	auto& List_62 = List_52.addChild<List>({
	  { mpid::Text, "Tags" }, 
	  { mpid::Foldable, 1 }, 
	  { mpid::Folded, 1 }
	});

	List_62.addChild<TagList>({
	  { mpid::ID, "addTagList" }, 
	  { mpid::Items, R"(API
Best Practice
Broadcaster
...
5 - great stuff)" }
	...
	List_52.addChild<JavascriptFunction>({
	  { mpid::CallOnNext, 0 }, 
	  { mpid::Code, R"(
...
document.getElementById("description").addEventListener("change", function()
{
	...
	List_48.addChild<JavascriptFunction>({
	  { mpid::CallOnNext, 1 }, 
...
if(state.newName.length > 0)
{
	...
	// Custom callback for page List_48
	List_48.setCustomCheckFunction([](Dialog::PageBase* b, const var& obj){
