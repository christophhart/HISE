...
juce::Image UIComponentDatabase::FloatingTileScreenshotProvider::getImage(const MarkdownLink& url, float )
{
    static constexpr char floatingTile_screenshotWildcard[] = "/images/floating-tile_";
    
	if (url.toString(MarkdownLink::UrlFull).startsWith(floatingTile_screenshotWildcard))
	{
		MemoryBlock mb;
