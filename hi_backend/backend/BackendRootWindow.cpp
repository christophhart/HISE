...
MarkdownPreview* BackendRootWindow::createOrShowDocWindow()
{
	if (docWindow == nullptr)
	{
		...
		AutogeneratedDocHelpers::createDocFloatingTile(docWindow->getRootFloatingTile());
	...
	return FloatingTileHelpers::findTileWithId<MarkdownPreviewPanel>(docWindow->getRootFloatingTile(), "Preview")->preview;
