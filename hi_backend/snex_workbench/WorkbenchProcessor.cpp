...
bool SnexWorkbenchEditor::perform(const InvocationInfo &info)
{
	...
	case FileNew:
	{
		...
		if (!fileName.isEmpty())
		{
			auto f = getRootFolder().getChildFile(fileName + ".xml");
	...
	case FileOpen:
	{
		FileChooser fc("Open File", getRootFolder(), "*.xml", true);
...
{
	auto root = getMainController()->getCurrentFileHandler().getSubDirectory(FileHandlerBase::DspNetworks);

	auto list = root.findChildFiles(File::findFiles, false, "*.xml");

	auto sourceDir = root.getChildFile("Build").getChildFile("Source");
