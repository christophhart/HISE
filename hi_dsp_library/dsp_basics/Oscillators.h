...
namespace pimpl
{
	struct tanh
	{
		SN_GET_SELF_AS_OBJECT(tanh);

		float getSample(float input)
		{
			return snex::hmath::tanh(input * gainFactor);
		}
	...
	template <typename T> struct wrapper
	{
		...
		{
			for (auto& s : data)
				s = getWrappedObject().getSample(s);
		}
...
using tanh = pimpl::wrapper<pimpl::tanh>;
}
}
