...
bool SampleMapEditor::perform (const InvocationInfo &info)
{
	...
	case NewSampleMap:		sampler->clearSampleMap(); return true;
	case LoadSampleMap:		{
							FileChooser f("Load new samplemap", GET_PROJECT_HANDLER(sampler).getWorkDirectory(), "*.xml;*.him");
	...
	case SaveSampleMapAsXml:		sampler->saveSampleMap(SampleMap::SaveMode::MultipleFiles); return true;
	case SaveSampleMapAsMonolith:	sampler->saveSampleMap(SampleMap::SaveMode::Monolith); return true;
	case ImportSfz:			{
