...
void AudioAnalysis::spec(float* d, float* freq, float* amp, float* time, 
...
{	
	...
	if ((freq) || (time)) {
		BlkDsp::cpxcopy(temp,d,hsize);
		x = 1e-6f*BlkDsp::cpxrms(temp,hsize);	// prevent large reassignment
		x = __max(x,2.0f*sqrtf(FLT_MIN));		// error for near 0 components
		BlkDsp::cpxprune(temp,x,x,hsize);		// allowing 120 dB dynamic range							
		BlkDsp::cpxinv(temp,hsize);				
	}
	...
	if (freq) {
		...
		BlkDsp::add(freq,freq+hsize,hsize);		// reassigned frequencies -> freq
	}
...
void AudioAnalysis::prespec(float* w, float* dw, float* rw, float* aic, int size)
{
	// scale window
	float x = BlkDsp::sum(w,size);
	BlkDsp::mul(w,1.0f/x,size+1);
	
	// calculate scaled first derivative of w
	BlkDsp::copy(dw,w,size+1);
	BlkDsp::diff(dw,size+1); 
	x = -1.0f/(2.0f*M_PI_FLOAT*static_cast<float>(size));
	BlkDsp::mul(dw,x,size);

	// calculate time ramped version of w
	BlkDsp::linear(rw,size+1,-0.5f,0.5f);
	BlkDsp::mul(rw,w,size);
...
void AudioAnalysis::matchingpursuit(float* weight, int* idx, int elements,
								float* dict, int atoms, float* data, int size)
{
	int i,j;
	float* m; m = new float[atoms];
	for (i=0; i<elements; i++) {
		for (j=0; j<atoms; j++) {m[j] = BlkDsp::dotp(dict+j*size,data,size);}
		idx[i] = BlkDsp::farthesti(m,0,atoms);
		weight[i] = m[idx[i]];
		BlkDsp::mac(data,dict+idx[i]*size,-weight[i],size);
	}
	delete[] m;
}
...
void AudioAnalysis::magspectorceps(float* d, int size)
{
	float m = d[BlkDsp::maxi(d,size+1)];
	BlkDsp::limit(d,size+1,m,1e-10f*m);
	BlkDsp::logabs(d,size+1);
	BlkDsp::realsymifft(d,size);
}
...
void AudioAnalysis::rcepstomagspec(float* d, int size)
{
	BlkDsp::realsymfft(d,size);
	BlkDsp::fexp(d,size+1);
}
...
void AudioAnalysis::spectomfcc(float* d, float* c, int size, int bands,
							   int cofs, float fs, float fmax, float fmin)
{
	...
	// take logarithm
	n = mspec[BlkDsp::maxi(mspec,bands)];
	BlkDsp::limit(mspec,bands,n,1e-10f*n);
	BlkDsp::logabs(mspec,bands);
...
cpx AudioAnalysis::lpdurbin(double* a, double* k, float* rm, int order)
{
	int i,j,ihalf;
...
void AudioAnalysis::lpanalyze(float* d, float* c, double* a, int size, int order)
{
	float* af; af = new float[order+1];
	BlkDsp::dtof(af,a,order+1);
	BlkDsp::fir(d, size, af, order, c);
	delete[] af;
}
...
//			block continuation data c[0..order-1]: zero fill for first call 
// output:	data d[0..size-1], c[0..order-1] for next call with continuous data
void AudioAnalysis::lpdsynth(float* d, double* c, double* a, int size, int order)
	{BlkDsp::iir(d, size, a, order, c);}
...
//			block continuation data c[0..order-1]: zero fill for first call 
// output:	data d[0..size-1], c[0..order-1] for next call with continuous data
void AudioAnalysis::lplsynth(float* d, double* c, double* k, int size, int order)
{
	int i,j; double x;
	order--;
	for (i=0; i<size; i++) {
...
void AudioAnalysis::lplsynth(float* d, double* c, double* kstart, double* kend,
							 int size, int order)
{
	int i,j; double x;
...
bool AudioAnalysis::lpctolsf(float* f, double* a, int order, int grid)
{
	int i,j, horder = order/2;
	grid = BlkDsp::nexthipow2(__max(grid,horder+1));
...
void AudioAnalysis::lpssynth(float* d, float* f, double* c, int size, int order)
{
...
void AudioAnalysis::lpssynth(float* d, float* fstart, float* fend, double* c,
							 int size, int order)
{
	...
	__m128d* ar = BlkDsp::sseallocm128d(horder);
	__m128d* ai = BlkDsp::sseallocm128d(horder);
	__m128d* wr = BlkDsp::sseallocm128d(horder);
	__m128d* wi = BlkDsp::sseallocm128d(horder);
	__m128d* cl = BlkDsp::sseallocm128d(order);
...
void AudioAnalysis::nlms(float* e, float* x, float* y, float* h, float* c,
...
void AudioAnalysis::costas(float* d, float* r, int size, float tbw, float dbw,
...
void AudioAnalysis::envelope(float* d, float* r, float &c, int size, 
							 float atime, float rtime, int type)
{
	...
	BlkDsp::copy(r, d, size);
	switch(type) {
	case 0:
		BlkDsp::abs(r, size);
		BlkDsp::add(r, ANTI_DENORMAL_FLOAT, size);
		break;
	case 1:
		BlkDsp::mul(r, r, size);
		BlkDsp::add(r, ANTI_DENORMAL_FLOAT, size);
	...
	default:
		BlkDsp::mul(r, r, size);
		BlkDsp::add(r, ANTI_DENORMAL_FLOAT*ANTI_DENORMAL_FLOAT, size);
		break;
	}
...
cpx AudioAnalysis::fundamental(float* d, int size, int type)
{
	...
	int i,j,imax; float ns1,ns2,n,rmax,nmax,temp,bias,bdec,yin; cpx res;
	int hsize = size>>1;
	...
	imax=1;
	if (type != 3) {while ((r[imax]>=0) && (imax<hsize)) {imax++;}}
	rmax=0; nmax = 1.0f; i=imax;
	if (type == 0) {					// McLeod/Wyvill
		while (i<hsize) {		
	...
	else if (type == 1) {				// Cauchy-Schwarz inequality
		while (i<hsize) {
			if ((r[i] > 0) && (r[i]>r[i-1]) && (r[i]>=r[i+1])) {
				temp = 0.25f*(r[i+1]-r[i-1]);
				temp = r[i] - temp*temp/(0.5f*(r[i+1]+r[i-1]) - r[i]);
	...
	else if (type == 2) {				// biased Cauchy-Schwarz inequality
		while (i<hsize) {
			if ((r[i] > 0) && (r[i]>r[i-1]) && (r[i]>=r[i+1])) {
				temp = 0.25f*(r[i+1]-r[i-1]);
				temp = r[i] - temp*temp/(0.5f*(r[i+1]+r[i-1]) - r[i]);
	...
	switch(type)
	{
	case 0:		// McLeod/Wyvill
	case 1:		// Cauchy-Schwarz inequality
	case 2:		// biased Cauchy-Schwarz inequality
...
int AudioAnalysis::fundverify(float* amp, int size, float fo)
{
	...
	for (i=0; i<12; i++) {
		...
		if (idxlo <= 0) {idx[i] = 0; a[i]=0;}
			...
			if (idxlo < hsize) {
				idx[i] = idxlo + BlkDsp::maxi(amp+idxlo,hsize-idxlo);
				a[i] = amp[idx[i]];
			}
			else {
			...
			idx[i] = idxlo + BlkDsp::maxi(amp+idxlo,idxhi-idxlo+1);
			a[i] = amp[idx[i]];
		}
		fc += finc;
	}
...
float AudioAnalysis::harmonics(	int* idx, int hms, float* amp,
...
//			continuation data c
float AudioAnalysis::zerocross(float* d, float &c, int size)
{
...
float AudioAnalysis::spectralflatness(float* d, int size, float f1, float f2)
{
	int n,lbin,hbin;
	...
	n = hbin - lbin + 1;
	mari = BlkDsp::mean(d+lbin,n);
	mgeo = BlkDsp::gmean(d+lbin,n);
...
float AudioAnalysis::ufc(float* d, int size, float pth)
{
	int i, idx = size;
	float x=0, plim = pth*BlkDsp::sum(d+1,size-1);
...
//					lower band limit)
// output:	return cutoff frequency relative to fs
float AudioAnalysis::lfc(float* d, int size, float pth)
{
	int i, idx = 0;
	float x=0, plim = pth*BlkDsp::sum(d+1,size-1);
...
float AudioAnalysis::spectralflux(float* d, float* c, int size)
{
	float n = BlkDsp::norm(d+1,size-1);
	float x = n*c[0];
	c[0] = n;
	if (x >= FLT_MIN) {x = sqrtf(fabsf(1.0f - BlkDsp::dotp(c+1,d+1,size-1)/x));}
	else {x = 0;}
	BlkDsp::copy(c+1,d+1,size-1);
	return x;
}		
...
void AudioAnalysis::transamp(float* d, float* r, float* c, int size,
...
//			continuation data c[0..size-1]
float AudioAnalysis::transspec(float* d, float* c, int size)
{
...
void AudioAnalysis::mqtracks(int* trkidx, int* trklink, int& tracks,
...
void AudioAnalysis::mqsyn(float* d, int size, float fs, float fe,
...
float AudioAnalysis::mqsyn(float* d, int size, float fs, float fe,
