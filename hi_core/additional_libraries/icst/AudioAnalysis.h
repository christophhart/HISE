...
public:
	...
	*/
	void analyseSpectrum(float* d, float* freq, float* amp, float* time, int size,
		float* w, float* dw, float* rw, float* temp, float* aic);

	/** Prepare a window for spectral analysis
	* input:	symmetric window w[size+1]. The data in the window will be altered.
	* output:	dw[size+1], rw[size+1], aic[4]
	*/
	void prepareWindow(float* w, float* dw, float* rw, float* aic, int size);
...
class RocketScienceAnalysis1 : public AudioAnalysisBase
{
	...
	static void analyseMatchingPursuit(float* weight, int* idx, int elements,
	...
	static void getRealCepstrumFromMagnitude(float* d, int size);
	...
	static void getMagnitudeSpectrumFromRealCepstrum(float* d, int size);
	...
	static Complex lpdurbin(double* a, double* k, float* rm, int order);

};

// ========================================================================================================================

class LpcAudioAnalysis : public AudioAnalysisBase
{
...
class RocketScienceAnalysis2 : public AudioAnalysisBase
{
	//***	adaptation ***
...
class EnvelopeFollowerAnalyis : public AudioAnalysisBase
{
	//***	feature extraction	***
	// envelope follower
...
public:
	...
	{
		McLeodWyvill = 0, ///< McLeod / Wyvill(intended for musical applications)
		CauchySchwarz, ///< Cauchy-Schwarz inequality (mathematical fundamental)
		BiasedCauchySchwarz, ///< biased Cauchy-Schwarz inequality (tunable compromise)
		YIN, ///< YIN (optimized for speech, seemingly withdrawn patent WO02097793)
		numNormalizationSchemes
	};
	...
	Complex getFundamentalFrequency(float* d, int size, NormalisationScheme type = NormalisationScheme::BiasedCauchySchwarz);

	/** Call this before you call getFundamentalFrequency with the maximal size you want to pass into.
	*
	*	It allocates working buffers. */
	void prepareFundamentalFrequencyBuffers(int size);
	...
	static int verifyFundamentalFrequency(float* amplitudeSpectrum, int size, float fundamentalFrequency);

	// extract harmonic spectrum and inharmonicity
	// input:	amplitude spectrum amp[0..size-1] as obtained from "analyseSpectrum"
	//			associated frequencies freq[0..size-1] as obtained from "analyseSpectrum" 
	//			fundamental frequency fo relative to fs (e.g. from "fundamental")
	...
	static float harmonics(int* idx, int hms, float* amplitudeSpectrum, float* frequencies,
		int size, float fo);
...
class VariousAudioAnalysis : public AudioAnalysisBase
{
...
class Resynthesis: public AudioAnalysisBase
{
	//***	resynthesis	  ***
