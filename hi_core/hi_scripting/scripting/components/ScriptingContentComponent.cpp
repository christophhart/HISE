/*
  ==============================================================================
    ...
    ScriptingContentComponent.cpp
    Created: 12 May 2015 9:44:59am
    Author:  Christoph
...
void ScriptContentComponent::setNewContent(ScriptingApi::Content *c)
{
	...
	for (int i = 0; i < contentData->components.size(); i++)
	{
		...
		if (ScriptingApi::Content::ScriptSlider *sc = dynamic_cast<ScriptingApi::Content::ScriptSlider*>(contentData->components[i].get()))
		{
			HiSlider *s;

			addAndMakeVisible(s = new HiSlider(String(sc->name)));

			s->setSliderStyle(Slider::RotaryHorizontalVerticalDrag);
			s->setTextBoxStyle(Slider::TextBoxRight, true, 80, 20);
			s->addListener(this);
			s->setValue(contentData->components[i]->value, dontSendNotification);

			s->setup(processor, i, String(sc->name));
			...
			{
				double min = GET_SCRIPT_PROPERTY(sc, min);
				double max = GET_SCRIPT_PROPERTY(sc, max);
				double step = sc->getScriptObjectProperty(ScriptingApi::Content::ScriptSlider::stepSize);

				s->setMode(sc->m, min, max, min + 0.5 * (max - min));
		...
		else if (ScriptingApi::Content::ScriptComboBox *scb = dynamic_cast<ScriptingApi::Content::ScriptComboBox*>(contentData->components[i].get()))
		{
			HiComboBox *cb;

			addAndMakeVisible(cb = new HiComboBox(String(scb->name)));

			cb->addItemList(scb->getItemList(), 1);
			cb->addListener(this);
			cb->setup(processor, i, String(scb->name));

			createdComponents.add(cb);

			cb->updateValue();
		}
		else if (ScriptingApi::Content::ScriptButton *sb = dynamic_cast<ScriptingApi::Content::ScriptButton*>(contentData->components[i].get()))
		{
			HiToggleButton *b;

			addAndMakeVisible(b = new HiToggleButton(String(sb->name)));
			b->setButtonText(String(sb->name));
			b->addListener(this);
			b->setToggleState((bool)sb->value, dontSendNotification);

			b->setup(processor, i, String(sb->name));
			...
			createdComponents.add(b);
		}
		else if (ScriptingApi::Content::ScriptLabel *sl = dynamic_cast<ScriptingApi::Content::ScriptLabel*>(contentData->components[i].get()))
		{
			Label *l;
			addAndMakeVisible(l = new MultilineLabel(String(sl->name)));
			...
			createdComponents.add(l);
		}
		else if (ScriptingApi::Content::ScriptTable *sl = dynamic_cast<ScriptingApi::Content::ScriptTable*>(contentData->components[i].get()))
		{
			TableEditor *t = new TableEditor(sl->getTable());

			t->setName(String(sl->name));

			addAndMakeVisible(t);

			createdComponents.add(t);

		}
		else if (ScriptingApi::Content::ModulatorMeter *m = dynamic_cast<ScriptingApi::Content::ModulatorMeter*>(contentData->components[i].get()))
		{
			ModulatorPeakMeter *pm = new ModulatorPeakMeter(m->targetMod);

			pm->setName(String(m->name));

			pm->setColour(Colour(0x22ffffff));

			addAndMakeVisible(pm);

			createdComponents.add(pm);

		}
		else if (ScriptingApi::Content::ScriptedPlotter *p = dynamic_cast<ScriptingApi::Content::ScriptedPlotter*>(contentData->components[i].get()))
		{
			Plotter *pl = new Plotter();

			pl->setName(String(p->name));
			...
			createdComponents.add(pl);

		}
		else if (ScriptingApi::Content::ScriptImage *img = dynamic_cast<ScriptingApi::Content::ScriptImage*>(contentData->components[i].get()))
		{
			ImageComponent *i = new ImageComponent();

			i->setName(String(img->name));

			i->setInterceptsMouseClicks(false, false);

			addAndMakeVisible(i);

			createdComponents.add(i);
		}
		else if (ScriptingApi::Content::ScriptPanel * panel = dynamic_cast<ScriptingApi::Content::ScriptPanel*>(contentData->components[i].get()))
		{
			BorderPanel *bp = new BorderPanel();

			bp->setName(String(panel->name));

			addAndMakeVisible(bp);
			createdComponents.add(bp);
		}
		else if (ScriptingApi::Content::ScriptSliderPack * ssp = dynamic_cast<ScriptingApi::Content::ScriptSliderPack*>(contentData->components[i].get()))
		{
			SliderPack *sp = new SliderPack(ssp->getSliderPack());

			sp->addListener(this);

			sp->setName(String(ssp->name));

			addAndMakeVisible(sp);
			createdComponents.add(sp);
		}
		else if (ScriptingApi::Content::ScriptAudioWaveform *form = dynamic_cast<ScriptingApi::Content::ScriptAudioWaveform*>(contentData->components[i].get()))
		{
			AudioSampleBufferComponent *asb = new AudioSampleBufferComponent(*const_cast<ScriptProcessor*>(getScriptProcessor())->getMainController()->getSampleManager().getAudioSampleBufferPool()->getCache());

			asb->setName(String(form->name));
			...
			addAndMakeVisible(asb);
			createdComponents.add(asb);
		}
		else
		{
			jassertfalse;
