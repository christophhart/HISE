...
struct ParserHelpers
{
	...
	struct TokenIterator
	{
		...
		{
			if (length > 0 && p > endPointer) return HiseJitTokens::eof;
...
#define NATIVE_JIT_COMPARE_KEYWORD(name, str) if (len == sizeof (str) - 1 && matchToken (HiseJitTokens::name, len)) return HiseJitTokens::name;
				NATIVE_JIT_KEYWORDS(NATIVE_JIT_COMPARE_KEYWORD)

					currentValue = String(p, end); p = end;
				currentString = String(p, end);
				return HiseJitTokens::identifier;
			}

			if (p.isDigit())
			{
				if (parseHexLiteral() || parseFloatLiteral() || parseOctalLiteral() || parseDecimalLiteral())
					return HiseJitTokens::literal;
...
#define NATIVE_JIT_COMPARE_OPERATOR(name, str) if (matchToken (HiseJitTokens::name, sizeof (str) - 1)) return HiseJitTokens::name;
			NATIVE_JIT_OPERATORS(NATIVE_JIT_COMPARE_OPERATOR)

				if (!p.isEmpty())
					location.throwError("Unexpected character '" + String::charToString(*p) + "' in source");

			return HiseJitTokens::eof;
		}
		...
		Identifier parseIdentifier()
		{
			...
			match(HiseJitTokens::identifier);
			return i;
		}
...
#define TYPE_MATCH(typeclass, typeinfo) HiseJITTypeHelpers::matchesType<typeclass>(typeinfo)
