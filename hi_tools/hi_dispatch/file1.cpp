...
void Logger::flush()
{
	messageQueue.flush(debugToConsole);
}
...
void Logger::printQueue(Queue& queue)
{
	...
	queue.flush([&](const Queue::FlushArgument& f)
	{
		...
		log(this, (uint8*)s.getCharPointer().getAddress(), s.length(), EventType::Log);
		return true;
...
bool Logger::debugToConsole(const Queue::FlushArgument& f)
{
	String s;
	s << f.source->getDispatchId() << ": ";

	if(f.eventType == EventType::Add)
		s << " ADD " << String(*(int*)(f.data));
	else if (f.eventType == EventType::Remove)
		s << " REM " << String(*(int*)(f.data));
	else
	{
		s << "[ ";
